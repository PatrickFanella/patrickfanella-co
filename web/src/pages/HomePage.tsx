    ...
    <>
      ...
      <section className="grid gap-8 border-b-2 border-stroke pb-12 pt-3 lg:grid-cols-[minmax(0,1.55fr)_minmax(18rem,0.65fr)] lg:items-end">
        <div>
          ...
          <h1 className="mt-5 max-w-[16ch] font-display text-[clamp(3.2rem,8vw,7.6rem)] font-bold uppercase leading-[0.84] tracking-[-0.055em] text-heading">
            <span className="block">
              Full-Stack <span className="text-accent-purple">Depth.</span>
            </span>{" "}
            <span className="block">
              User <span className="text-accent-pink">minded.</span>
            </span>{" "}
            <span className="block">
              Product <span className="text-accent-green">ownership.</span>
            </span>
          </h1>
          <p className="mt-6 max-w-[36ch] text-[1.08rem] leading-relaxed text-ink-soft sm:text-[1.2rem]">
            I build operational products from data model and API design through accessible React
            interfaces and production delivery—using Go, Python, TypeScript, PostgreSQL, search, and
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <Link className={secondaryButtonClass} to="/resume">
              Download my résumé
            </Link>
            <Link className={textLinkClass} to="/contact">
              Discuss a role →
            </Link>
          </div>
        </div>
        ...
        <aside
          ...
          aria-label="Availability and core strengths"
        >
          <p className={monoLabelClass}>Available now</p>
          <p className="mt-4 font-display text-2xl font-bold leading-tight text-heading">
            Chicago or remote
          </p>
          <ul className="mt-5 grid list-none gap-2 p-0">
            {focusAreas.map((area) => (
              <li
                className="border-2 border-stroke bg-surface px-3 pt-[calc(0.5rem+0.5px)] pb-[calc(0.5rem-0.5px)] font-mono text-xs uppercase tracking-[0.12em]"
                key={area}
              >
                {area}
              </li>
          ...
          <a className={`${textLinkClass} mt-5`} href="mailto:fanella.patrick@gmail.com">
      ...
      <section className="py-12" aria-labelledby="featured-heading">
        <div className="mb-8 grid gap-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
          <div>
            ...
            <h2
              className="mt-5 font-display text-[clamp(2.6rem,5vw,4.6rem)] font-bold uppercase leading-[0.9] tracking-[-0.04em] text-heading"
              id="featured-heading"
            >
              Selected work.
            </h2>
          ...
          <p className="max-w-[38ch] text-ink-soft">
            Projects showing production delivery, user-minded design, and systems under active
            development.
          </p>
        </div>
        ...
        {status === "loading" ? (
          <RouteState
            ariaLive="polite"
            description="Loading the selected case studies."
            label="Loading"
            role="status"
            title="Loading featured work."
          />
        ) : null}
        {status === "error" ? (
          <RouteState
        ...
        {status === "success" ? (
          <>
            ...
            <div className="mt-8">
              <Link className={secondaryButtonClass} to="/archive">
                View all projects
              </Link>
            </div>
          </>
